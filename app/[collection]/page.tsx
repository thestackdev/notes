...
export default function Page({ params: { collection } }: CollectionProps) {
  ...
  const { isLoading: dataLoading, error: dataError } = useQuery(
    `get-items-${collection}`,
    () =>
      fetch(`/api/collections/collection?id=${collection}`).then((res) =>
        res.json()
      ),
    {
      onSuccess: (data) => {
  ...
  const updateData = useMutation(
    async (params: { id: string; content: any }) => {
  ...
  useEffect(() => {
    ...
    const cancel = setTimeout(() => {
      updateData.mutate({
        id: collection,
        content: { columns: columns, tasks: tasks },
      });
    }, 1000);
  ...
  return (
    <div className="flex w-full lg:ml-[300px] overflow-x-auto">
      <DndContext
        sensors={sensors}
      ...
      >
        <div className="flex p-8 gap-4">
          ...
          <Button className="w-[350px]" onClick={() => createNewColumn()}>
            <PlusIcon />
            Add Column
          </Button>
        </div>
